...
export default function DashboardLayout({ channels, userId }: Props) {
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0f0f0f] overflow-hidden">
      {/* ── Header ── */}
      <header className="flex items-center h-14 px-4 bg-[#202020] border-b border-[#303030] flex-shrink-0 z-40">
        {/* Left */}
        <div className="flex items-center gap-4 w-60 flex-shrink-0">
          <button onClick={() => setSidebarOpen(!sidebarOpen)}
            className="w-9 h-9 flex items-center justify-center rounded-full hover:bg-[#303030] transition-colors text-[#aaaaaa] hover:text-white text-lg">
            ☰
          </button>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <button className="w-9 h-9 flex items-center justify-center rounded-full hover:bg-[#303030] transition-colors text-[#aaaaaa] hover:text-white">
            <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
          ...
          <button className="flex items-center gap-2 bg-[#303030] hover:bg-[#404040] text-white text-xs font-medium px-4 py-2 rounded-full transition-colors ml-1">
            <span className="text-base leading-none">+</span>
            <span>作成</span>
          </button>
          {/* Avatar */}
          <div
            className="w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0 cursor-pointer ml-1"
      ...
      <div className="flex flex-1 min-h-0">
        {/* Sidebar */}
        <aside
          className="flex flex-col bg-[#202020] border-r border-[#303030] flex-shrink-0 overflow-y-auto transition-all duration-200"
          style={{ width: sidebarOpen ? 240 : 72 }}
        >
          {/* Channel info */}
          <div className={`flex flex-col items-center py-6 px-3 border-b border-[#303030] ${!sidebarOpen ? "px-2" : ""}`}>
          ...
          <div className="py-2 border-t border-[#303030]">
            {[
              { icon: <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5"><path d="M19.14,12.94c0.04-0.3,0.06-0.61,0.06-0.94c0-0.32-0.02-0.64-0.07-0.94l2.03-1.58c0.18-0.14,0.23-0.41,0.12-0.61 l-1.92-3.32c-0.12-0.22-0.37-0.29-0.59-0.22l-2.39,0.96c-0.5-0.38-1.03-0.7-1.62-0.94L14.4,2.81c-0.04-0.24-0.24-0.41-0.48-0.41 h-3.84c-0.24,0-0.43,0.17-0.47,0.41L9.25,5.35C8.66,5.59,8.12,5.92,7.63,6.29L5.24,5.33c-0.22-0.08-0.47,0-0.59,0.22L2.74,8.87 C2.62,9.08,2.66,9.34,2.86,9.48l2.03,1.58C4.84,11.36,4.8,11.69,4.8,12s0.02,0.64,0.07,0.94l-2.03,1.58 c-0.18,0.14-0.23,0.41-0.12,0.61l1.92,3.32c0.12,0.22,0.37,0.29,0.59,0.22l2.39-0.96c0.5,0.38,1.03,0.7,1.62,0.94l0.36,2.54 c0.05,0.24,0.24,0.41,0.48,0.41h3.84c0.24,0,0.44-0.17,0.47-0.41l0.36-2.54c0.59-0.24,1.13-0.56,1.62-0.94l2.39,0.96 c0.22,0.08,0.47,0,0.59-0.22l1.92-3.32c0.12-0.22,0.07-0.47-0.12-0.61L19.14,12.94z M12,15.6c-1.98,0-3.6-1.62-3.6-3.6 s1.62-3.6,3.6-3.6s3.6,1.62,3.6,3.6S13.98,15.6,12,15.6z" /></svg>, label: "設定" },
              { icon: <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-7 12h-2v-2h2v2zm0-4h-2V6h2v4z" /></svg>, label: "フィードバック" },
            ].map((item) => (
              <button
                key={item.label}
